...
public class FsPft
{
    ...
    public Font loadPftFont(File file, String encoding) throws Exception
    {
        ...
        IndexColorModel cm = new IndexColorModel(1, 2, new byte[] { (byte) 255, 0 }, new byte[] { (byte) 255, 0 },
                new byte[] { (byte) 255, 0 });
        ...
        for (int i = 0; i < glyphCount - 1; i++)
        {
            ...
            {
                BufferedImage glyph = new BufferedImage(pftFont.getRealWidth(i), font.getHeight(),
                        BufferedImage.TYPE_BYTE_BINARY, cm);
                writeToImage(glyph, helper.getGlyphBytes(i));
                font.getGlyphTable()[i] = glyph;
            }
        }
        BufferedImage glyph = new BufferedImage(helper.getDefaultGlyphWidth(), font.getHeight(),
